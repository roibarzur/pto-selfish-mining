...
class StateDependantBoltzmannExploration(ExplorationMechanism):
    ...
    def explore(self, action_values: Dict[int, float], state_visit_count: int) -> int:
        ...
        temperature = self.temperature / state_visit_count
        return np.random.choice(actions, p=scipy.special.softmax(values / temperature))
